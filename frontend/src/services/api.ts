...
// Use Vite dev proxy (configured in `vite.config.ts`) or an explicit API URL if provided.
// By default we point to the same origin so that `/api/...` requests go through the proxy
// to the backend on port 3001 during development.
